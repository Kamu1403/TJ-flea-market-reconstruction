...
def get_item_pics():
    ...
    pic_path = os.listdir(os.path.join(item_blue.static_folder, 'resource/item_pic/1/pic'))
    if len(pic_path):
        pic_list = list()
        for i in pic_path:
            pic_list.append(url_for('item.static',filename=f'resource/item_pic/{item_id}/pic/{i}'))
...
def post_item_info():
    ...
    if len(data["urls"]) == 0:
        ...
        with open(url_for('item.static',filename=f'resource/default/test.jpg'), "rb") as f:
            g = open(url_for('item.static',filename=f'resource/item_pic/{new.id}/head/0.jpg'), "wb")
            g.write(f.read())
            g.close()
            h = open(url_for('item.static',filename=f'resource/item_pic/{new.id}/pic/0.jpg'), "wb")
            h.write(f.read())
            h.close()
    else:
        ...
        with open(url_for('item.static',filename=f'resource/temp/{current_user.id}/{head_pic}.jpg'), "rb") as f:
            g = open(url_for('item.static',filename=f'resource/item_pic/{new.id}/head/0.jpg'), "wb")
            g.write(f.read())
            g.close()
        for i,j in enumerate(data["urls"]):
            with open(url_for('item.static',filename=f'resource/temp/{current_user.id}/{j["MD5"]}.jpg'),"rb") as f:
                g = open(url_for('item.static',filename=f'resource/item_pic/{new.id}/pic/{i}.jpg'),"wb")
                g.write(f.read())
                g.close()
        #将所有的图片转到用户对应文件夹
...
def post_item_pic():
    ...
    try:
        ...
        createPath(os.path.join(item_blue.static_folder, f'resource/temp/{current_user.id}/'))
        with open(url_for('item.static',filename=f'resource/temp/{current_user.id}/{md5code}.jpg'), "wb") as f:
            f.write(file_byte)
    except Exception as e:
        return make_response_json(400, f"上传失败\n{str(e)}{repr(e)}")
